...
from src.opt_control.OptiMPC import OptiCasadi
...
class PlaneMPC(OptiCasadi):
    ...
    def compute_obstacle_cost(self, cost, x_position, y_position) -> None:
        ...
        for i, x in enumerate(obs_x_vector):
            ...
            avoidance_cost = obs_avoidance_weight * \
                ca.sumsqr(obstacle_constraints)
    ...
    def compute_dynamic_threats_cost(self, cost: float) -> None:
        ...
        for threat in threat_list:
            ...
            threat_cost = self.dynamic_threat_params['weight'] * \
                ca.sumsqr(distance_cost)
            cost += threat_cost
