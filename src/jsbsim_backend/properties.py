...
from src.jsbsim_backend.utils import AttributeFormatter
"""
...
heading_rad = BoundedProperty(
    'attitude/heading-true-rad', 'yaw [rad]', -math.pi, math.pi)
...
pdot_radps2 = BoundedProperty('accelerations/pdot-rad_sec2',
                              'roll acceleration [rad/s^2]', -2 * math.pi, 2 * math.pi)
qdot_radps2 = BoundedProperty('accelerations/qdot-rad_sec2',
                              'pitch acceleration [rad/s^2]', -2 * math.pi, 2 * math.pi)
rdot_radps2 = BoundedProperty('accelerations/rdot-rad_sec2',
                              'yaw acceleration [rad/s^2]', -2 * math.pi, 2 * math.pi)
altitude_rate_fps = Property(
    'velocities/h-dot-fps', 'Rate of altitude change [ft/s]')
# not certain about this one
airspeed = Property('velocities/vt-fps', 'True aircraft airspeed [ft/s]')
...
aileron_left_rad = Property(
    'fcs/left-aileron-pos-rad', 'left aileron deflection [rad]')
aileron_right_rad = Property(
    'fcs/right-aileron-pos-rad', 'right aileron deflection [rad]')
aileron_combined_rad = Property(
    'fcs/effective-aileron-pos', 'combined effective aileron deflection [rad]')
...
# autopilot commands
heading_des = BoundedProperty(
    'ap/heading_setpoint', 'desired heading [deg]', -180, 180)
level_des = BoundedProperty('ap/altitude_setpoint',
                            'desired altitude [ft]', -1400, 85000)
heading_switch = BoundedProperty(
    'ap/heading_hold', 'engage heading mode [bool]', 0, 1)
level_switch = BoundedProperty(
    'ap/altitude_hold', 'engage alt hold [bool]', 0, 1)
attitude_switch = BoundedProperty(
    'ap/attitude_hold', 'engage att hold [bool]', 0, 1)
wing_level_switch = BoundedProperty(
    'fcs/wing-leveler-ap-on-off', 'engage wing leveler [bool]', -1, 0)

# simulation
sim_dt = Property('simulation/dt', 'JSBSim simulation timestep [s]')
sim_time_s = Property('simulation/sim-time-sec', 'Simulation time [s]')
trim_switch = BoundedProperty(
    'simulation/do_simple_trim', 'engage trimming [bool]', 0, 1)
