...
from src.jsbsim_backend.simulator import FlightDynamics
from scipy import interpolate
from src.guidance_control.navigation import LocalNavigation
from src.conversions import mps_to_ktas, meters_to_feet, feet_to_meters
...
class TECSParameters():
    def __init__(self) -> None:
       # Vehicle specific params
        ...
        # Lower limit of true airspeed demand [m/s]
        self.tas_min = 0.0
        self.pitch_max = 0.0              # Max pitch angle [rad]
        self.pitch_min = 0.0              # Min pitch angle [rad]
        # Normalized throttle at level flight [0,1]
        ...
        # Altitude error inverse time constant [1/s]
        self.altitude_error_gain = 0.0
        # Gain from altitude demand derivative to climb rate
        self.altitude_setpoint_gain_ff = 0.0

        # Airspeed control params
        # Percentage of airspeed tracking errors [0,1]
        self.tas_error_percentage = 0.0
        # Airspeed error inverse time constant [1/s]
        self.airspeed_error_gain = 0.0
        ...
        # Damping gain for throttle demand [s]
        self.throttle_damping_gain = 0.0
        self.throttle_slewrate = 0.0      # Throttle demand slew rate [1/s]

        # Load factor parameters
        # Gain from load factor to energy rate [m²/s³]
        self.load_factor_correction = 0.0
