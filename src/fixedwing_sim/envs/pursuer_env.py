...
class PursuerEnv(gymnasium.Env):
    ...
    def init_pursuers(self, num_pursuers: int) -> list[PursuerInterface]:
        ...
        while init_count < num_pursuers:
            ...
            pursuer_geo_pos = local_to_global_position(
    ...
    def init_attitude_action_space(self) -> spaces.Box:
        ...
        z_min = self.rl_control_constraints['z_min']
        z_max = self.rl_control_constraints['z_max']

        u_psi_min = self.rl_control_constraints['heading_cmd_min']
        u_psi_max = self.rl_control_constraints['heading_cmd_max']

        v_cmd_min = self.rl_control_constraints['v_cmd_min']
        v_cmd_max = self.rl_control_constraints['v_cmd_max']

        self.heading_commands = np.arange(u_psi_min, u_psi_max, np.deg2rad(5))
        self.altitude_commands = np.arange(z_min, z_max, 5)
        self.velocity_commands = np.arange(v_cmd_min, v_cmd_max, 1)

        action_space = spaces.MultiDiscrete(
            [len(self.heading_commands),
    ...
    def __get_observation(self) -> dict:
        ego_obs = self.backend_interface.get_observation()
        norm_ego_obs = self.map_real_observation_to_normalized_observation(
            ego_obs)
        # update observation space with pursuer distance
        for pursuer in self.pursuers:
            pursuer_obs = pursuer.get_observation()
            norm_pursuer_obs = self.map_real_observation_to_normalized_observation(
                pursuer_obs)
            norm_pursuer_pos = norm_pursuer_obs[:3]

            norm_distance = self.compute_distance(
                norm_ego_obs[:3], norm_pursuer_pos)
    ...
    def get_reward(self) -> tuple:
        ...
        for i, pursuer in enumerate(self.pursuers):
            ...
            pursuer_obs = pursuer.get_observation()
            # self.map_real_observation_to_normalized_observation(pursuer_obs)
            norm_pursuer_obs = pursuer_obs
            norm_pursuer_pos = norm_pursuer_obs[:3]
            old_distance = self.distance_history[i]
            ...
            # get 2d unit vectors
            ego_unit_vector = ego_obs['ego'][:2] / \
                np.linalg.norm(ego_obs['ego'][:2])
            pursuer_unit_vector = norm_pursuer_obs[:2] / \
                np.linalg.norm(norm_pursuer_obs[:2])
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, dict]:
        ...
        UPDATE THIS will be now discrete
