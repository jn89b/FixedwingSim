...
class SimpleKinematicEnv(gymnasium.Env):
    ...
    def init_action_space(self) -> spaces.Box:
        ...
        phi_min = self.control_constraints['u_phi_min']
        phi_max = self.control_constraints['u_phi_max']
        theta_min = self.control_constraints['u_theta_min']
        theta_max = self.control_constraints['u_theta_max']
        psi_min = self.control_constraints['u_psi_min']
        psi_max = self.control_constraints['u_psi_max']
        v_min = self.control_constraints['v_cmd_min']
        v_max = self.control_constraints['v_cmd_max']

        self.phi_commands = np.arange(phi_min, phi_max, np.deg2rad(5))
        self.theta_commands = np.arange(theta_min, theta_max, np.deg2rad(5))
        self.psi_commands = np.arange(psi_min, psi_max, np.deg2rad(5))
        self.v_commands = np.arange(v_min, v_max, 1)

        action_space = spaces.MultiDiscrete([len(self.phi_commands),
                                             len(self.theta_commands),
                                             len(self.psi_commands),
                                             len(self.v_commands
                                                 )])
    ...
    def discrete_to_continuous(self, action: np.ndarray) -> np.ndarray:
        ...
        phi_cmd: float = self.phi_commands[action[phi_idx]]
        theta_cmd: float = self.theta_commands[action[theta_idx]]
        psi_cmd: float = self.psi_commands[action[
            psi_idx]]
        vel_cmd: float = self.v_commands[action[vel_idx]]

        return np.array([phi_cmd, theta_cmd, psi_cmd, vel_cmd])
        ...
        if self.use_pursuers:
            ...
            for p in self.pursuers:
                ...
                pursuer_norm_heading = self.map_real_to_norm(
                    ...
                    self.state_constraints['psi_min'],
                    heading_pursuer)
                norm_heading_diff = abs(
                    self.norm_start_state[5] - pursuer_norm_heading)
    ...
    def __get_reward(self, current_state: np.ndarray) -> tuple:
        ...
        distance_from_goal = self.compute_distance(
            current_position, self.goal_state)
    ...
    def __get_evader_reward(self, current_state: np.ndarray) -> tuple:
        ...
        if caught:
            ...
            # + min_norm_distance + min_dot_product#(sum_reward/len(self.pursuers))
            reward = -100
            return reward, done
        else:
            done = False
            # sum_reward/len(self.pursuers) + 1
            reward = 1 + min_dot_product + min_norm_distance
            return reward, done
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, Dict]:
        ...
        real_action = self.discrete_to_continuous(action)
        ...
        for i in range(every_one_second):
            ...
            if self.use_pursuers:
                ...
                for p in self.pursuers:
                    ...
                    pitch_cmd = np.clip(
                        error_pitch, -np.deg2rad(20), np.deg2rad(20))
                    roll_cmd = np.clip(
                        error_los, -np.deg2rad(30), np.deg2rad(30))
                    ...
                    pursuer_next_state = p.rk45(
                        pursuer_state, pursuer_action, self.dt)
