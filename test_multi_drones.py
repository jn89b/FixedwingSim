...
class PursuerInterface(CLSimInterface):
    ...
    # def calculate_turn_angle(self, pursuer_mag_vel, delta_v:np.ndarray) -> float:
    #     """
    #     Calculate the turn angle required for the velocity change.
        
    #     :param pursuer_vel: Tuple (vx, vy) representing the current velocity of the pursuer.
    #     :param delta_v: Tuple (Δvx, Δvy) representing the required change in velocity.
    #     :return: Turn angle in degrees.
    #     """
    #     # Calculate the desired velocity
    #     # v_desired = (pursuer_vel[0] + delta_v[0], pursuer_vel[1] + delta_v[1])
    #     v_desired = pursuer_mag_vel + delta_v
        
    #     # # Calculate the dot product of v and v_desired
    #     # dot_product = 
    #     # dot_product = pursuer_vel[0]*v_desired[0] + pursuer_vel[1]*v_desired[1]
        
    #     # # Calculate the magnitudes of v and v_desired
    #     # mag_v = self.vector_magnitude(pursuer_vel)
    #     mag_v_desired = self.vector_magnitude(v_desired)
        
    #     # Calculate the cosine of the turn angle
    #     cos_theta = dot_product / (mag_v * mag_v_desired)
        
    #     # Calculate the turn angle in radians and then convert to degrees
    ...
    def cartesian_to_navigation_radians(self, 
        ...
        """
        # # Convert Cartesian angle to degrees first for easier understanding and calculation
        # cartesian_angle_degrees = math.degrees(cartesian_angle_radians)
        
        # # Convert the Cartesian angle (in degrees) to navigation system angle (in degrees)
        # navigation_angle_degrees = (450 - cartesian_angle_degrees) % 360
        
        # # Convert the navigation angle back to radians
        # navigation_angle_radians = math.radians(navigation_angle_degrees)
        
        # return navigation_angle_radians
    ...
    def pro_nav(self, target_states:np.ndarray) -> tuple:
        ...
        los_rate = np.array([target_vx - pursuer_vx, target_vy - pursuer_vy])
        ...
        # v_cmd = pursuer_vel_mag + self.nav_constant * los_rate
        return los, los_mag_vel
    ...
    def set_command(self, heading_cmd_rad:float,
        ...
        print("heading cmd deg: ", np.rad2deg(heading_cmd_rad))
...
for i in range(N_steps):
    ...
    for p in pursuer_list:
        ...
        turn_cmd, v_cmd = p.pro_nav(evader_states)
        
        pursuer_position = p.get_observation()[0:3]
        pursuer_vel = p.get_observation()[-1] + v_cmd
        print("pursuer vel: ", pursuer_vel)
        p.set_command(turn_cmd, 15, 50)
        ...
        evader.set_command(np.deg2rad(135), 15, 50) 
...
#plot quivers for pursuers
# for p in pursuer_list:
#     states = p.get_observation()
#     position = states[0:3]
#     heading = states[6]
#     ax.quiver(position[0], position[1], position[2], 
#               np.cos(heading_cmd), np.sin(heading_cmd), 
#               0, length=10, normalize=True, color='r')
...
ax.plot(pursuer_x_history, pursuer_y_history, pursuer_z_history, color='r', alpha=0.5)
ax.plot(evader_x_history, evader_y_history, evader_z_history, color='b', alpha=0.5)
# ax.set_xlim(-100, 100)
# ax.set_ylim(-100, 100)
# ax.quiver(evader_position[0], evader_position[1], evader_position[2],
#           np.cos(heading_cmd), np.sin(heading_cmd),
#           0, length=10, normalize=True, color='b')
ax.set_xlabel('X')
ax.set_ylabel('Y')
